      ...
      <NavigationProvider value={{ push: pushToHistory, back: handleBack, stack: navStack }}>
        <div className="min-h-screen flex flex-col font-sans bg-[color:var(--bg)] text-[color:var(--ink)]">
          ...
          <div className="flex-1 flex w-full max-w-[1600px] mx-auto">
            ...
            <main className="flex-1 min-w-0 px-4 sm:px-6 lg:px-10 py-8 lg:py-10 cp-mobile-main-pad cp-page">
              {activePage === 'admin' ? (
                ...
                <PrivacyPolicyView onBack={handleBack} onContactClick={navigateToContact} initialTab={legalTab} />
              ) : activePage === 'notFound' ? (
                <NotFoundView onGoHome={navigateToHome} onOpenSearch={() => setIsSearchOpen(true)} onSelectTool={navigateToTool} />
              ) : activePage !== 'home' ? (
                <TrustPageView page={activePage as TrustPageKey} onBack={handleBack} onContactClick={navigateToContact} />
              ) : activeTool ? (
                <div className="max-w-6xl mx-auto">{renderTool(activeTool)}</div>
              ) : (
          ...
          <Footer
            ...
            onOpenBugReport={() => setIsBugReportOpen(true)}
            onNavigateTrust={navigateToTrustPage}
          />
          <SearchModal isOpen={isSearchOpen} onClose={() => setIsSearchOpen(false)} onSelectTool={navigateToTool} />
          <SitemapModal isOpen={isSitemapModalOpen} onClose={() => setIsSitemapModalOpen(false)} onSelectTool={navigateToTool} />

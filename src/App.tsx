...
export const App: React.FC = () => {
  ...
  const renderTool = (tool: ToolDef) => {
    ...
    const view = views[tool.id] || <CalculatorsView tool={tool} onBackToHome={navigateToHome} onSelectRelated={navigateToTool} />;
    return view;
  };
    ...
    <CurrencyProvider>
      <NavigationProvider value={{ push: pushToHistory, back: handleBack, stack: navStack }}>
        <div className="min-h-screen flex flex-col font-sans selection:bg-[color:var(--brand)] selection:text-white bg-[color:var(--bg)] text-[color:var(--ink)]">
          ...
          <BugReportModal isOpen={isBugReportOpen} onClose={() => setIsBugReportOpen(false)} detail={bugReportData} onNavigateContact={navigateToContact} />
        </div>
      </NavigationProvider>
    </CurrencyProvider>
  );

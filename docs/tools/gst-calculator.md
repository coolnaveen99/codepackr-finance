...
**Live calculator:** [finance.codepackr.com/gst-calculator](https://finance.codepackr.com/gst-calculator)
...
Use this calculator when you need a transparent estimate for **gst calculator**. It is intended for planning and comparison: change one assumption at a time, observe the effect on the outputs, and keep the assumptions with any result you save. The result is an estimate, not a quote, approval, tax filing, investment guarantee, or professional opinion.

## 2. Inputs

The main inputs are:

- **Taxable base amount**: enter the value that describes your current scenario. Keep its unit and time period consistent with the other fields.
- **GST rate**: enter the value that describes your current scenario. Keep its unit and time period consistent with the other fields.
- **Inclusive or exclusive mode**: enter the value that describes your current scenario. Keep its unit and time period consistent with the other fields.
- **Intra-state or inter-state transaction type**: enter the value that describes your current scenario. Keep its unit and time period consistent with the other fields.
...
- **Pre-tax amount**: use this as a decision signal, then verify it against the detailed schedule, table, or chart.
- **GST amount**: use this as a decision signal, then verify it against the detailed schedule, table, or chart.
- **Invoice total**: use this as a decision signal, then verify it against the detailed schedule, table, or chart.
- **CGST/SGST or IGST split**: use this as a decision signal, then verify it against the detailed schedule, table, or chart.
...
> Exclusive tax = base * rate; inclusive tax = total * rate / (1 + rate).

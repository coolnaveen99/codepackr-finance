...
**Live calculator:** [finance.codepackr.com/ctc-to-in-hand-calculator](https://finance.codepackr.com/ctc-to-in-hand-calculator)
...
Use this calculator when you need a transparent estimate for **ctc to in-hand salary calculator**. It is intended for planning and comparison: change one assumption at a time, observe the effect on the outputs, and keep the assumptions with any result you save. The result is an estimate, not a quote, approval, tax filing, investment guarantee, or professional opinion.

## 2. Inputs

The main inputs are:

- **Annual CTC**: enter the value that describes your current scenario. Keep its unit and time period consistent with the other fields.
- **Basic salary**: enter the value that describes your current scenario. Keep its unit and time period consistent with the other fields.
- **Allowances**: enter the value that describes your current scenario. Keep its unit and time period consistent with the other fields.
...
- **Tax regime**: enter the value that describes your current scenario. Keep its unit and time period consistent with the other fields.
- **Payroll assumptions**: enter the value that describes your current scenario. Keep its unit and time period consistent with the other fields.
...
- **Gross salary**: use this as a decision signal, then verify it against the detailed schedule, table, or chart.
- **Deductions**: use this as a decision signal, then verify it against the detailed schedule, table, or chart.
- **Annual tax estimate**: use this as a decision signal, then verify it against the detailed schedule, table, or chart.
- **Monthly take-home pay**: use this as a decision signal, then verify it against the detailed schedule, table, or chart.
- **A CTC component breakdown**: use this as a decision signal, then verify it against the detailed schedule, table, or chart.
...
> Take-home pay = gross earnings - employee deductions - estimated payroll taxes; employer benefits are separated from cash salary.
...
- [Salary Hike & Increment Calculator](./salary-hike-calculator.md) - Evaluate annual appraisal raises, job switch offer jumps, and inflation-adjusted real purchasing power gains.
- [Gratuity Calculator](./gratuity-calculator.md) - Calculate statutory terminal separation gratuity, tenure rounding, and Section 10(10) tax exemptions.
- [HRA Exemption Calculator](./hra-calculator.md) - Compute tax-exempt House Rent Allowance (HRA) under Section 10(13A) Rule 2A, compare metro vs non-metro limits, and estimate tax savings.

...
**Category:** Loans & Debt  
**Live calculator:** [finance.codepackr.com/loan-amortization-calculator](https://finance.codepackr.com/loan-amortization-calculator)

Generate complete annual and monthly loan amortization ledgers with principal reduction and cumulative interest tracking.

## 1. What this calculator answers

Use this calculator when you need a transparent estimate for **loan amortization calculator**. It is intended for planning and comparison: change one assumption at a time, observe the effect on the outputs, and keep the assumptions with any result you save. The result is an estimate, not a quote, approval, tax filing, investment guarantee, or professional opinion.

## 2. Inputs

The main inputs are:

- **Principal**: enter the value that describes your current scenario. Keep its unit and time period consistent with the other fields.
...
- **Term**: enter the value that describes your current scenario. Keep its unit and time period consistent with the other fields.
- **Payment frequency**: enter the value that describes your current scenario. Keep its unit and time period consistent with the other fields.
- **Start date or period labels**: enter the value that describes your current scenario. Keep its unit and time period consistent with the other fields.
...
- **A period-by-period ledger showing opening balance**: use this as a decision signal, then verify it against the detailed schedule, table, or chart.
- **Payment**: use this as a decision signal, then verify it against the detailed schedule, table, or chart.
- **Principal**: use this as a decision signal, then verify it against the detailed schedule, table, or chart.
- **Interest**: use this as a decision signal, then verify it against the detailed schedule, table, or chart.
- **Closing balance**: use this as a decision signal, then verify it against the detailed schedule, table, or chart.
...
> Interest for a period = opening balance * periodic rate; principal = payment - interest; closing balance = opening balance - principal.
